...
def build_bestfits_section(rows: list[dict], limit_per_path: int = 3, display_groups: dict | None = None) -> str:
    """Build HTML for Section 2: Best Fits grouped by display group (or career path fallback)."""
    if not rows:
        return '<p class="empty-message">No companies to explore yet.</p>'

    # Group by display group (if provided) or raw role_family
    path_groups: dict[str, list[dict]] = {}
    if display_groups is not None:
        # Build reverse mapping: path_label -> group_name
        path_to_group: dict[str, str] = {}
        for group_name, path_labels in display_groups.items():
            for label in path_labels:
                path_to_group[label] = group_name
        for r in rows:
            raw_path = r.get('role_family', '').strip() or 'Other'
            group = path_to_group.get(raw_path, 'Other')
            path_groups.setdefault(group, []).append(r)
    else:
        for r in rows:
...
def build_watch_list_section(rows: list[dict]) -> str:
    """Build HTML for Worth Monitoring section: watch_list companies with score >= 50."""
    if not rows:
        return ''
  ...
  <td><span class="status-badge {css_class}">{label}</span></td>
    ...
    return f'''<div class="section watch-list-section" id="watchListSection">
  <div class="section-header collapsible-header" data-collapse-target="watchListContent">
    ...
    <span class="badge-muted">{len(rows)} companies</span>
  </div>
  <div class="collapsible-content" id="watchListContent" style="display:none">
  <table class="pipeline-table watch-list-table">
  <thead>
  <tr>
    <th>Company</th>
    <th>Score</th>
...
def build_html(merged: list[dict], full_mode: bool) -> str:
    ...
    if search_config_path.exists():
        try:
            ...
            display_groups = search_config.get('display_groups') or None
...
def _get_js() -> str:
  ...
  document.getElementById('pillPipeline').addEventListener('click', function() {
    ...
    if (content.style.display === 'none') {
      content.style.display = 'block';
      var toggle = content.previousElementSibling && content.previousElementSibling.querySelector('.section-toggle');
  ...
  var collapsibleHeaders = document.querySelectorAll('.collapsible-header[data-collapse-target]');
