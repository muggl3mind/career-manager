...
After tracker operations, suggest the next logical action:

- After `followup` shows stale applications: "Want me to draft follow-up text for these?" (compose using `job-search/templates/follow-up-email.md`, user copies to their email client)
- After `add`: no suggestion needed (user just added it)
- After `list` or `report`: if any applications need follow-up, mention it
