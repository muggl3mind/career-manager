...
# Import named targets list for coverage check
sys.path.insert(0, str(JOB_SEARCH / 'scripts' / 'ops'))
try:
    from web_prospecting import PROSPECTING_PATHS
except ImportError:
    PROSPECTING_PATHS = []
...
def check_named_target_coverage(seen: dict) -> list[dict]:
    checks = []
    all_targets = []
    for p in PROSPECTING_PATHS:
        for name in p.get('named_targets', []):
            all_targets.append(name)

    if not all_targets:
        checks.append({
            'check': 'named_target_coverage',
            'status': 'warn',
            'detail': 'Could not load PROSPECTING_PATHS — skipping coverage check',
        })
        return checks

    total = len(all_targets)
    covered = sum(1 for t in all_targets if t.lower() in seen)
    missing = [t for t in all_targets if t.lower() not in seen]
    ...
    checks.append({
        'check': 'named_target_coverage',
        'status': status,
        'detail': f'{covered}/{total} ({pct:.0f}%) named targets in seen-companies.json',
    })
    if missing:
        checks.append({
            'check': 'named_targets_missing',
            'status': 'info',
            'detail': ', '.join(missing[:10]) + (f' (+{len(missing)-10} more)' if len(missing) > 10 else ''),
        })
    return checks
...
def run_health_check(as_json: bool = False) -> int:
    ...
    all_checks.extend(check_named_target_coverage(seen))
