...
class TestProspectingResultsScoringSchema:
    ...
    def test_merge_copies_llm_score_to_new_company(self, tmp_path):
        ...
        assert row['role_family'] == 'AI-Native Fintech', \
            f"Unexpected role_family: {row['role_family']!r}"
        # 'AI-Native Fintech' is not in the real search-config canonical paths,
        # so unknown_path flag is appended by _validate_role_family.
        assert 'growth_unknown' in row['llm_flags'], \
            f"Expected growth_unknown in llm_flags, got: {row['llm_flags']!r}"
        assert 'unknown_path' in row['llm_flags'], \
            f"Expected unknown_path in llm_flags for unrecognized path, got: {row['llm_flags']!r}"
